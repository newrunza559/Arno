...
def login_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                return redirect('booking:home')  # แทนที่ด้วย URL ที่ต้องการให้ผู้ใช้ไปหลังจากเข้าสู่ระบบ
            else:
...
def home(request):
    ...
    return render(request, 'home.html', {'rooms': rooms, 'foods': foods})
...
def mark_room_as_available(request, room_id):
    try:
        ...
        room.save()
        return redirect('booking:home')  # <<< แก้เป็น home ตรงนี้
